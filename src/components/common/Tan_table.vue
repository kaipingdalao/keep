<template>
  <div id="Tan_table">
    <div class="table-border">
      <div class="table-box">
        <div class="t-header">
          <table>
            ...
            <tr>
              ...
              <th class="t-fixed-right" style="min-width: 170px;max-width: 170px; right: 0px">option</th>
            </tr>
            </thead>
          </table>
        </div>
        ...
        <table>
          ...
          <tr v-for="(item, index) in props.t_body">
            ...
            <td class="t-fixed-left" style="min-width: 170px;max-width: 170px; right: 0px">
              <i v-for="i in options.handle" @click="i.fun(item.id,item.title)">{{i.title}}</i>
            </td>
          </tr>
          </tbody>
        </table>
      ...
      <div class="page">
        <ul>
          <li @click="nowPage > 1 && selectPage(nowPage-1)"><</li>
          <li v-for="(item, index) in options.pageSize"
              @click="selectPage(index+1)"
              :class="{'active-page': nowPage == index+1}"
          >
            {{index +1}}
          </li>
          <li @click="nowPage < options.pageSize && selectPage(nowPage+1)">></li>
        </ul>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    setup(props, {emit}) {
      ...
      return {
        ...
        width_sum, colgroup_direction, props, selectPage, nowPage
      }
    }
  }
</script>
